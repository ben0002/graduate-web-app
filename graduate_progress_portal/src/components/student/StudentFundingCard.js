...
export default function StudentFundingCard() {
  ...
  var makeFundingCards = _ => {
    return fundings.map( (funding, idx) => { return(
      <div style={{ marginX: 'auto', width: '90%', position: 'relative', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', borderBottom: '1px solid lightgray'}} onClick={_ => {setModal(funding); setMakeNew(false)}} key={`funding-${idx}`}>
