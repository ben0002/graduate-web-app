...
async def upload_student_file(file: UploadFile, db: Session = Depends(get_db)):    
    ...
    #except Exception as e:
        #raise HTTPException(status_code=500, detail=f"There was an error processing the ffrom fastapi import FastAPI, Depends, HTTPException, Request, Response, Cookie, UploadFile
from fastapi.middleware.cors import CORSMiddleware
from fastapi.responses import RedirectResponse
from cas import CASClient
...
async def upload_student_file(file: UploadFile, db: Session = Depends(get_db)):    
    try:
        ...
        raise HTTPException(status_code=422, detail={"error_message": str(value_error), 
                                                     "problematic_row": value_error.row_data}) 
    #except Exception as e:
        #raise HTTPException(status_code=500, detail=f"There was an error processing the f